...
func (h *StationHandler) Create(c fiber.Ctx) error {
	...
	if body[0] == '[' {
		...
		for _, s := range stations {
			if s.ID < 0 || s.ID > 999 {
				return api.Error(c, api.CodeValidationError, "Station ID must be 0-999")
			}
		}
		...
		case string:
			if v == "" {
				// Generate ID not supported here unless auto-increment?
                                // User UI sends empty string if not provided.
                                // If it is "e.g. 001", it handles string.
				// For now assuming ID is REQUIRED or handled.
                // The UI passes `singleId`.
